...
class CategoryService:
    ...
    def upsertCategoryImage(self, category_id: int, imageFile: UploadFile) -> str:
        try:
            ...
            data, count = self.supabase.table("categories").select("id").eq("id", category_id).execute()
            if not data[1]:
            ...
            self.supabase.table("categories").update({"image_url": url}).eq("id", category_id).execute()
            
            return "Category image updated successfully"
        except Exception as e:
            logger.error(f"Error updating category image: {str(e)}")
            raise HTTPException(status_code=500, detail=f"Error updating category image: {str(e)}")

    def deleteCategoryImage(self, category_id: int) -> str:
        try:
            # Obtener la imagen actual
            data, count = self.supabase.table("categories").select("image_url").eq("id", category_id).execute()
            if not data[1]:
                raise HTTPException(status_code=404, detail="Category not found")
            ...
            if not image_url:
                raise HTTPException(status_code=400, detail="Category has no image to delete")
            ...
            if success:
                ...
                self.supabase.table("categories").update({"image_url": None}).eq("id", category_id).execute()
